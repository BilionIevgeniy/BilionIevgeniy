# 👋 Hello, I'm Ievgen Bilion  

### Frontend Developer | React & TypeScript | Production-Focused Engineer  

<br>

## 🚀 Professional Profile

I am a **Frontend Developer with 3.5+ years of commercial experience**, building scalable web applications in the **Health-Tech industry**.

My core expertise lies in developing production-grade React applications with a strong focus on **clean architecture, performance, and maintainability**.

Currently, I am expanding my expertise in modern frontend engineering and AI-powered applications at **Developer Akademie (Germany)** — strengthening my skills in Angular, advanced architecture, and AI integration.

- 💻 **Core Expertise:** React, TypeScript, Redux Toolkit, Angular
- 🏗️ **Architecture:** Modular design, scalable frontend structure
- ⚡ **Performance:** Bundle optimization, lazy loading, async flows  
- 🧪 **Testing Mindset:** Jest, RTL, structured CI workflows  
- 🤖 **AI Integration:** Practical experience integrating LLM APIs and automation tools  
- 🌍 **Languages:** English (B2), German (B2), Ukrainian & Russian (Native)  

---

## 🛠 Tech Stack & Toolset

### 🏆 Commercial Experience (3.5+ Years)

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="javascript" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" alt="typescript" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original-wordmark.svg" alt="react" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/redux/redux-original.svg" alt="redux" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original-wordmark.svg" alt="nodejs" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original-wordmark.svg" alt="postgresql" width="56" height="56"/>

### 🚀 Advanced Upskilling

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/angularjs/angularjs-original.svg" alt="angular" width="45" height="45"/>&nbsp;&nbsp;
<img src="https://www.vectorlogo.zone/logos/supabase/supabase-icon.svg" alt="supabase" width="45" height="45"/>&nbsp;&nbsp;
<img src="https://www.vectorlogo.zone/logos/jestjsio/jestjsio-icon.svg" alt="jest" width="45" height="45"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" alt="docker" width="45" height="45"/>&nbsp;&nbsp;

---

## 💼 Professional Experience

**Fullstack Developer | OSSystem (Health-Tech Industry)**  
2020 – 2024  

- Built and maintained complex React frontends  
- Developed REST APIs and microservices using Node.js / NestJS  
- Designed scalable database structures (PostgreSQL)  
- Collaborated in cross-functional teams  
- Delivered production-ready features in real-world environments  

---

## 🎓 Education & Professional Development

- **AI Software Developer** | Developer Akademie, Munich (Februar 2026 – Present (Expected August 2026)  
  *Focus: Angular (v20), Supabase, AI-powered web apps, RAG systems*

- **Fullstack Developer** | Commercial Experience (2020–2024)  
  *PERN stack, Microservices, Production Systems*

- **Engineering Degree** | Odessa State Academy of Civil Engineering (2007–2012)  
  *Strong analytical and systems-thinking foundation*

---

## 📫 Get in Touch

<a href="mailto:ievgenbilion@gmail.com">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" height="32"/>
</a>&nbsp;&nbsp;
<a href="https://www.linkedin.com/in/ievgen-bilion/">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="32"/>
</a>

---

## 🧠 Philosophy

> Building clean, scalable, production-ready frontend systems — with an engineering mindset.
