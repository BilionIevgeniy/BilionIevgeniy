# 👋 Hello, I'm Ievgen Bilion

### Web Developer | AI Software Engineer in Training @ Developer Akademie

<br>

## 🚀 Professional Profile
I am a results-driven **Web Developer with 3.5+ years of professional experience**, specializing in building scalable web applications and microservices. My background is rooted in the **Health-Tech industry**, where I developed robust solutions using the **PERN stack** (PostgreSQL, Express, React, Node.js) and **NestJS**.

Currently, I am undergoing an intensive **AI Software Engineering** specialization at **Developer Akademie (Germany)**. I am bridgeing the gap between classic fullstack development and the future of **AI-driven automation**.

- 💻 **Core Expertise:** React, Redux, Node.js, NestJS, and TypeScript.
- 🏗️ **Architecture:** Microservices, RESTful APIs, and Clean Code.
- 🤖 **AI Integration:** Leveraging LLMs (Gemini, ChatGPT), GitHub Copilot, and automation tools (n8n) to build "smart" applications.
- 🌍 **Languages:** English (B2), German (B2), Ukrainian & Russian (Native).

## 🛠 Tech Stack & Toolset

### 🏆 Professional Experience (3.5+ Years)
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="javascript" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" alt="typescript" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original-wordmark.svg" alt="react" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/redux/redux-original.svg" alt="redux" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original-wordmark.svg" alt="nodejs" width="56" height="56"/>&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original-wordmark.svg" alt="postgresql" width="56" height="56"/>

### 🧪 Advanced Upskilling @ Developer Akademie
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/angularjs/angularjs-original.svg" alt="angular" width="45" height="45"/>&nbsp;&nbsp;
<img src="https://www.vectorlogo.zone/logos/supabase/supabase-icon.svg" alt="supabase" width="45" height="45"/>&nbsp;&nbsp;
<img src="https://www.vectorlogo.zone/logos/jestjsio/jestjsio-icon.svg" alt="jest" width="45" height="45"/>&nbsp;&nbsp;
<img src="https://www.vectorlogo.zone/logos/google_cloud/google_cloud-icon.svg" alt="gemini" width="45" height="45"/>&nbsp;&nbsp;

---

## 🎓 Education & Professional Journey
- **KI-Softwareentwickler (AI Software Developer)** | Developer Akademie, Munich (2025-2026)
  *Focus: Web Applications with AI & Automation, Angular v20, Supabase, and RAG.*
- **Fullstack Developer** | OSSystem, Ukraine (2020-2024)
  *Key Project: Leal.Health (Health-Tech). Built microservices and complex UIs.*
- **Engineer Background** | Odessa State Academy of Civil Engineering (2007-2012)
  *Heating System Planner - providing a strong logical and analytical foundation.*

---

## 📊 GitHub Insights
![Ievgen's Stats](https://github-readme-stats.vercel.app/api?username=BilionIevgeniy&show_icons=true&theme=radical)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=BilionIevgeniy&layout=compact&theme=radical)

---

## 📫 Get in Touch
<a href="mailto:ievgenbilion@gmail.com">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" height="32"/>
</a>&nbsp;&nbsp;
<a href="https://www.linkedin.com/in/ievgen-bilion/">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="32"/>
</a>

---
*"Blending 3.5 years of software engineering with the power of Artificial Intelligence."*
